...
const TripCourse = () => {
    ...
    const [randomLodging, setRandomLodging] = useState(null);
    ...
    const fetchLodgingData = async () => {
        try {
            ...
            setLodgings(response.data.results.slice(0, 4));
            setLoading(false);

            if (response.data.results.length > 0) {
                const randomIndex = Math.floor(Math.random() * response.data.results.length);
                setRandomLodging(response.data.results[randomIndex]);
            }
        } catch (error) {
            console.error("숙소 데이터를 불러오는 중 오류 발생: ", error);
            setLoading(false);
        }
    };

    const fetchTripNews = async () => {
        try {
            const response = await axios.get('http://localhost:8080/api/tripnews');
            console.log(response.data);
            const newsItems = response.data.response.body.items.item;

            if (newsItems) {
                setTripNews(newsItems.slice(0, 5));
            } else {
                console.error("여행 뉴스 데이터가 없습니다.");
            }
        } catch (error) {
            console.error("여행 뉴스를 불러오는 중 오류 발생: ", error);
        }
    };

    const fetchTourCourses = async () => {
        try {
            const response = await axios.get('http://localhost:8080/api/tourcourses');
            const items = response.data.response.body.items.item;

            if (items) {
                setTourCourses(items);
                setRandomCourse(items[Math.floor(Math.random() * items.length)]);
            } else {
                console.error("여행 코스 데이터가 없습니다.");
            }
            
        } catch (error) {
            console.error("여행 코스 데이터를 불러오는 중 오류 발생: ", error);
        } finally {
            setLoading(false);
        }
    };

    const refreshRandomCourse = () => {
        if (tourCourses.length > 0) {
            setRandomCourse(tourCourses[Math.floor(Math.random() * tourCourses.length)]);
        }
    };
    ...
    const Tripnews = ({ title, description, image }) => {
        return (
            <div className='trip-item'>
                <img src={image || 'https://via.placeholder.com/150'} alt={title} className='trip-image' />
                    ...
                    {loading ? (
                        ...
                        randomCourse && (
                            <div className="courses">
                                <img src={randomCourse.firstimage || 'https://via.placeholder.com/150'} alt={randomCourse.title} className="course-image" />
                                <h3 className="course-title">{randomCourse.title}</h3>
                                <p className="course-description">{randomCourse.addr1}</p>
                                <button className="refresh-button" onClick={refreshRandomCourse}>다음 랜덤 코스 보기</button>
                        </div>
                        )
                    )}
                </div>
            ...
            <div className='lodging'>
                ...
                <div className="slider-controls">
                    ...
                    {loading ? (
                        ...
                        lodgings.map((lodge, index) => (
                            <Card
                                key={index}
                                image={lodge.photos ? `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${lodge.photos[0].photo_reference}&key=${G_API_KEY}` : "https://via.placeholder.com/400"}
                                title={lodge.name}
                                description={lodge.vicinity}
                            />
                        ))
                    )}
                        </div>
